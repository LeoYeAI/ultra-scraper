...
Usage:
  scrape.py fetch <url> [--css SELECTOR] [--xpath SELECTOR] [--output FILE] [--format FORMAT] [--stealth] [--dynamic] [--solve-cloudflare] [--ultra-stealth] [--headless] [--impersonate BROWSER] [--timeout SECONDS]
  scrape.py extract <url> [--css SELECTOR] [--xpath SELECTOR] [--output FILE] [--format FORMAT] [--stealth] [--dynamic] [--solve-cloudflare] [--ultra-stealth] [--headless] [--impersonate BROWSER] [--timeout SECONDS]
  scrape.py screenshot <url> [--output FILE] [--stealth] [--dynamic] [--ultra-stealth] [--fullpage] [--headless]
  scrape.py links <url> [--stealth] [--dynamic] [--ultra-stealth] [--filter PATTERN]
...
Fetcher tiers (auto-escalation on failure):
  1. Fetcher         — fast HTTP, impersonates real browser TLS
  2. DynamicFetcher  — full headless browser, renders JS/SPAs
  3. StealthyFetcher — anti-bot bypass (Cloudflare Turnstile, CAPTCHA)
  4. CloakBrowser    — C++ source-level Chromium patches, strongest stealth
                       (optional, install with: pip install cloakbrowser)
"""
...
# ---------------------------------------------------------------------------
# CloakBrowser integration (optional, tier-4 stealth)
# ---------------------------------------------------------------------------
...
│                                                             │
│  Then retry with:  --ultra-stealth                          │
└─────────────────────────────────────────────────────────────┘
"""


def is_cloakbrowser_available():
    """Check if cloakbrowser is importable in any available Python."""
    ...
    try:
        result = subprocess.run(
            [sys.executable, '-c', 'import cloakbrowser'],
...
def fetch_with_cloakbrowser(url, css=None, xpath=None, timeout=None,
                             proxy=None, humanize=False, headless=True):
    """Fetch a page using CloakBrowser (tier-4 stealth).

    Returns a dict with keys: html, text, elements (list of dicts).
    Raises ImportError if cloakbrowser is not installed.
    """
    try:
        from cloakbrowser import launch
    except ImportError:
        raise ImportError('cloakbrowser is not installed')
    ...
    try:
        ...
        nav_kwargs = {}
        if timeout:
            nav_kwargs['timeout'] = int(timeout) * 1000  # Playwright uses ms
        ...
        if css:
            ...
            handles = page.query_selector_all(f'xpath={xpath}')
            for h in handles:
                elements.append({
                    'text': (h.inner_text() or '').strip(),
                    'html': h.inner_html(),
        ...
        return {'html': html, 'text': text, 'elements': elements}
    finally:
        browser.close()


def format_cloakbrowser_output(result, fmt='json', url=''):
    """Format CloakBrowser fetch result into the standard output format."""
    ...
    if fmt == 'text':
        ...
        if elements:
            return '\n'.join(e['html'] for e in elements)
        return result.get('html', '')

    elif fmt == 'markdown':
        try:
            from markdownify import markdownify as md
            html = '\n'.join(e['html'] for e in elements) if elements else result.get('html', '')
            return md(html)
        except ImportError:
            return '\n'.join(e['text'] for e in elements) if elements else result.get('text', '')
        ...
        for e in elements:
            writer.writerow([e['text'], e.get('attributes', {}).get('href', '')])
        return buf.getvalue()
        ...
        return json.dumps({'url': url, 'html_length': len(result.get('html', ''))}, indent=2)


# ---------------------------------------------------------------------------
# Standard fetcher selection
# ---------------------------------------------------------------------------
...
def cmd_fetch(args):
    ...
    # Tier 4: CloakBrowser (explicit flag or auto-fallback after stealth failure)
    if getattr(args, 'ultra_stealth', False):
        if not is_cloakbrowser_available():
            print(CLOAKBROWSER_INSTALL_MSG, file=sys.stderr)
            sys.exit(1)
        print('[ultra-stealth] Using CloakBrowser (C++ source-level stealth)...', file=sys.stderr)
        ...
        output = format_cloakbrowser_output(result, fmt, url=args.url)
        write_output(output, getattr(args, 'output', None))
        return

    # Tiers 1-3: standard Scrapling fetchers
    try:
        page = fetch_page(args)
    except Exception as e:
        err_msg = str(e).lower()
        # Detect signs of strong anti-bot blocking
        blocked_signals = [
            'cloudflare', 'turnstile', 'captcha', 'forbidden', '403',
            'access denied', 'blocked', 'challenge', 'kasada', 'datadome'
        ]
        is_blocked = any(sig in err_msg for sig in blocked_signals)

        if is_blocked and not getattr(args, 'stealth', False):
            # Auto-escalate to stealth and retry
            print(f'[auto-escalate] Blocked ({e}), retrying with --stealth...', file=sys.stderr)
            args.stealth = True
            args.solve_cloudflare = True
            try:
                page = fetch_page(args)
            except Exception as e2:
                # Stealth also failed — suggest CloakBrowser
                print(f'[stealth] Also failed: {e2}', file=sys.stderr)
                if is_cloakbrowser_available():
                    print('[auto-escalate] Retrying with CloakBrowser (tier 4)...', file=sys.stderr)
                    result = fetch_with_cloakbrowser(
                        args.url,
                        css=getattr(args, 'css', None),
                        xpath=getattr(args, 'xpath', None),
            ...
            # Already in stealth mode and failed — suggest CloakBrowser
...
def main():
    ...
    p_fetch.add_argument('--ultra-stealth', action='store_true',
                         help='Use CloakBrowser (C++ source-level stealth, requires: pip install cloakbrowser)')
    p_fetch.add_argument('--humanize', action='store_true',
                         help='Human-like mouse/keyboard behavior (CloakBrowser only)')
    p_fetch.add_argument('--proxy', help='Proxy URL (e.g. http://user:pass@host:port or socks5://...)')

    # extract
    p_extract = subparsers.add_parser('extract', help='Extract text content (default: text format)')
    add_common_args(p_extract)
    p_extract.add_argument('--ultra-stealth', action='store_true',
                           help='Use CloakBrowser (C++ source-level stealth)')
    p_extract.add_argument('--humanize', action='store_true')
    p_extract.add_argument('--proxy', help='Proxy URL')
